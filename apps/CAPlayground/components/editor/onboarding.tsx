...
export function EditorOnboarding({ showLeft, showRight }: { showLeft: boolean; showRight: boolean }) {
  ...
  const steps: Step[] = useMemo(() => [
    ...
    },
    {
      ...
      title: "状态",
      body: "切换壁纸的不同状态，如锁定、解锁或休眠状态。您可以在这里创建状态过渡。",
      placement: "right",
    },
    {
      id: "canvas",
      ...
      title: "画布",
      body: "拖拽、调整大小和预览图层。使用 ⌘/+ / ⌘/- 缩放，Shift+拖拽 平移。",
      placement: "bottom",
    },
    {
      id: "inspector",
    ...
    },
    {
      ...
      title: "设置",
      body: "配置吸附行为，查看键盘快捷键，并重置此引导以再次查看。",
      placement: "bottom",
    },
  ], []);
    ...
    <div className="fixed inset-0 z-50 pointer-events-none">
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <Button variant="outline" size="sm" onClick={goPrev}>上一步</Button>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={isLast ? finish : goNext}>{isLast ? '完成' : '下一步'}</Button>
          </div>
        </div>
      </div>
    </div>
