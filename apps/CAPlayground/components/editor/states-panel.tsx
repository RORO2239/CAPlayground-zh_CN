...
const getStateDisplayName = (state: string): string => stateDisplayNames[state] || state;
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Dialog open={viewAllOpen} onOpenChange={setViewAllOpen}>
            ...
            <DialogHeader>
              <DialogTitle>状态过渡概览</DialogTitle>
            </DialogHeader>
            <div className="flex-1 overflow-y-auto space-y-6 pr-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {allStates.map(state => (
                    <Badge key={state} variant={state === active ? "default" : "secondary"}>
                      {state === 'Base State' ? '基础状态' : getStateDisplayName(state)}
                    </Badge>
                  ))}
                </div>
              </div>
              ...
              {Object.keys(overridesByLayer).length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold mb-3">按图层分类的状态覆盖</h3>
                        ...
                        <div className="space-y-2">
                          {Object.entries(stateData).map(([stateName, overrides]) => (
                            <div key={stateName} className="text-xs">
                              <Badge variant="outline" className="mb-1">{getStateDisplayName(stateName)}</Badge>
              ...
              {Object.keys(overridesByLayer).length === 0 && (
                <div className="text-center text-muted-foreground py-8">
                  <p className="text-sm">尚未配置状态过渡。</p>
                  <p className="text-xs mt-1">选择一个状态并修改图层属性以创建过渡。</p>
                </div>
              )}
            </div>
            </DialogContent>
        ...
        <div className="text-sm rounded-lg border bg-card shadow-sm divide-y flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-auto">
            ...
            {states.map((s) => (
              ...
              >
                <div className="truncate flex-1">{getStateDisplayName(s)}</div>
              </button>
            ))}
          </div>
        </div>
