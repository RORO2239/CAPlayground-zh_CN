        ...
        <div className="space-y-1">
          ...
          >
            <SelectTrigger className="h-8 text-xs">
              <SelectValue placeholder="选择关键路径" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="position">位置 (position)</SelectItem>
              <SelectItem value="position.x">位置 X (position.x)</SelectItem>
              <SelectItem value="position.y">位置 Y (position.y)</SelectItem>
              <SelectItem value="transform.rotation.x">旋转 X (transform.rotation.x)</SelectItem>
              <SelectItem value="transform.rotation.y">旋转 Y (transform.rotation.y)</SelectItem>
              <SelectItem value="transform.rotation.z">旋转 Z (transform.rotation.z)</SelectItem>
              <SelectItem value="opacity">不透明度 (opacity)</SelectItem>
              <SelectItem value="bounds">边界 (bounds)</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="space-y-1 col-span-2">
          ...
          <div className="flex items-center gap-2 h-8">
            <Switch
              ...
              disabled={!animEnabled}
            />
            <span className="text-xs text-muted-foreground">重复时反向播放</span>
          </div>
        </div>
        <div className="space-y-1 col-span-2">
          <Label htmlFor="anim-duration">时长 (秒)</Label>
          <Input
            id="anim-duration"
            type="number"
            step="0.01"
        ...
        <div className="space-y-1 col-span-2">
          ...
          <div className="flex items-center gap-2 h-8">
            <Switch
              ...
              disabled={!animEnabled}
            />
            <span className="text-xs text-muted-foreground">关闭时需指定总重复时间。</span>
          </div>
        </div>
        {(((selectedBase as any)?.animations?.infinite ?? 1) as number) !== 1 && (
          <div className="space-y-1 col-span-2">
            <Label htmlFor="anim-repeat">重复时长 (秒)</Label>
            <Input
              id="anim-repeat"
              type="number"
              step="0.01"
      ...
      <div className="space-y-2">
        <div className="space-y-1">
          <Label>
            {(() => {
              ...
              if (kp.startsWith('transform.rotation')) return '值 (度)';
              if (kp === 'position') return '值 (CGPoint)';
              if (kp === 'opacity') return '值 (百分比)';
              if (kp === 'bounds') return '值 (CGRect)';
              return '值 (数字)';
            })()}
          </Label>
          <div className="text-xs text-muted-foreground">
            {(() => {
              const kp = ((selectedBase as any)?.animations?.keyPath ?? 'position') as KeyPath;
              if (kp.startsWith('transform.rotation')) return '旋转的动画值（以度为单位）。';
              if (kp === 'position') return '动画值为 x, y 坐标。';
              if (kp === 'opacity') return '动画值为不透明度百分比。';
              if (kp === 'bounds') return '动画值为宽度、高度尺寸。';
              return '动画值为数字。';
            })()}
          </div>
          <Button
        ...
        <div className={`space-y-2 ${animEnabled ? '' : 'opacity-50'}`}>
                ...
                <>
                  {values.map((pt, idx) => (
                    <div key={idx} className="grid grid-cols-3 gap-2 items-end">
                      <div className="space-y-1">
                        <Label className="text-xs">{kp === 'position' ? 'X' : '宽度'}</Label>
                        <Input
                          type="number"
                          step="1"
                          className="h-8"
                      ...
                      <div className="space-y-1">
                        <Label className="text-xs">{kp === 'position' ? 'Y' : '高度'}</Label>
                        <Input
                          type="number"
                          step="1"
                          className="h-8"
                  ...
                  <div key={idx} className="grid grid-cols-2 gap-2 items-end">
                    <div className="space-y-1">
                      <Label className="text-xs">
                        {kp === 'position.x' ? 'X' : kp === 'position.y' ? 'Y' : kp === 'opacity' ? '不透明度' : '度数'}
                      </Label>
          ...
          {(((selectedBase as any)?.animations?.values || []) as any[]).length === 0 && (
            <div className="text-xs text-muted-foreground">暂无关键值。点击"+ 添加关键值"添加第一个关键帧。</div>
          )}
        </div>
      </div>
